...
import scripts.results_evaluation as re
import scripts.vbench_evaluation as ve
...
def init_paths(input_ns):
    ...
    input_ns.inverse_path = f'{const.GENERATED_DATA_PATH}/inverses/{input_ns.save_folder}/{input_ns.preprocess_name}_{input_ns.model_id}_{input_ns.grid_size}x{input_ns.grid_size}_{input_ns.pad}'
    input_ns.control_path = f'{const.GENERATED_DATA_PATH}/controls/{input_ns.save_folder}/{input_ns.preprocess_name}_{input_ns.grid_size}x{input_ns.grid_size}_{input_ns.pad}'
...
def run(input_ns):
    ...
    print(f'Grid count: {input_ns.sample_size}')

    controlnet_class = AMAC
    ...
    if 'optical_flow' in str(input_ns.preprocess_name):
        res_vid, control_vid = CN(input_dict)
    elif '-' in str(input_ns.controlnet_conditioning_scale):
    ...
    yaml_dict['shuffling_type'] = f"{yaml_dict['shuffling_type']}-tome{yaml_dict['local_merge_ratio']}"
    with open(f'{input_ns.save_path}/config.yaml', 'w') as yaml_file:
        yaml.dump(yaml_dict, yaml_file)

    # compute metrics and save scores
    re.save_scores(input_ns.positive_prompts, input_ns.save_path, input_ns.save_folder, input_ns.shuffling_type)
    ve.evaluate_and_save(input_ns.save_path, "subject_consistency")
...
if __name__ == '__main__':
    ...
    # for right path in debug case
    import utils.constants as const
    from scripts.sd_controlnet_pipeline import AMAC
