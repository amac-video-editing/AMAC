...
def unmake_grid(grids_tensor, nrow, padding=0):
    """
    grids_tensor: Tensor of shape [B, C, H_total, W_total]
    nrow: number of images per row used in make_grid
    padding: padding used in make_grid
    """
    B, C, H_total, W_total = grids_tensor.shape

    H = (H_total + padding) // nrow - padding
    W = (W_total + padding) // nrow - padding
    ...
    for grid_ind in range(B):
        ...
        for i in range(nrows):
            for j in range(ncols):
                top = i * (H + padding)
                left = j * (W + padding)
                patch = grids_tensor[grid_ind, :, top:top + H, left:left + W]
                imgs.append(patch)
        img_lst += imgs

    return torch.stack(img_lst)


def make_grids_from_frames(frames_list, nrow):
    N = len(frames_list)

    frames_per_grid = nrow**2

    grids = []
    for g in range(N // frames_per_grid):
        start = g * frames_per_grid
        end = start + frames_per_grid
        frames = frames_list[start:end]

        # Arrange frames into a grid
        rows = []
        for i in range(nrow):
            row_frames = frames[i * nrow:(i + 1) * nrow]
            row = torch.cat(list(row_frames), dim=2)  # concatenate along width
            rows.append(row)
        grid = torch.cat(rows, dim=1)  # concatenate rows along height

        grids.append(grid)

    return torch.stack(grids)  # Shape [B, C, H_total, W_total]
