...
def bipartite_soft_matching_randframe(metric: torch.Tensor, 
                                      F: int, threshold: float, unm_pre: int, generator: torch.Generator,
    ...
    with torch.no_grad():
        ...
        randf = torch.randint(0, target_stride, torch.Size([1]), generator=generator, device=generator.device)
        ...
        if align_batch:
            ...
            #threshold
            sorted_values, sorted_indices = torch.sort(node_max, dim=-1, descending=True)
            mask = sorted_values < threshold  # Boolean mask where values < threshold
            r = int(mask.int().argmax(dim=-1)[0])  # index of smallest index where value threshold
            edge_idx = sorted_indices[..., None]
