...
import utils.token_merging as tm
import utils.permute_frames as pf
...
class AMAC(nn.Module):
    ...
    @torch.no_grad()
    def denoising_step(self, latents, control_image, text_embeddings, t, guidance_scale, current_sampling_percent, return_x0=False):

        # allows variant token merging ratio
        if hasattr(self, 'local_merge_ratio') and self.local_merge_ratio == 'decreasing':
            self.unet._tome_info['args']['local_merge_ratio'] = float(0.1 + (t / 1000) * 0.4)
        ...
        prev_step = self.scheduler.step(noise_pred, t, latents)
        latents = prev_step['prev_sample']

        if return_x0:
            x0_hat = prev_step['pred_original_sample']
    ...
    def preprocess_control_grid(self, image_pil):
        list_of_image_pils = fu.pil_grid_to_frames(image_pil, grid_size=self.grid)
        list_of_pils = []
        prev_frame_pil = list_of_image_pils[0]
        for frame_pil in list_of_image_pils:
            new_frame_pil = pu.pixel_perfect_process(np.array(frame_pil, dtype='uint8'), self.preprocess_name, prev_image=prev_frame_pil)
            prev_frame_pil = frame_pil
            list_of_pils.append(new_frame_pil)
    ...
    @torch.no_grad()
    def batch_denoise(self, latents, control_image, indices, t, guidance_scale, current_sampling_percent, return_x0=False):
        ...
        for idx in range(len(control_split)):
            ...
            if return_x0:
                latents, x0_hat = self.denoising_step(latents_split[idx], control_split[idx], txt_embed, t, guidance_scale, current_sampling_percent, return_x0=return_x0)
                x0_l.append(x0_hat)
            else:
        ...
        if self.sample_size > self.total_frame_number // self.grid_frame_number:
            self.sample_size -= 1
            latents = self.order_all_frames(latents, indices, repeat=True)
            controls = self.order_all_frames(controls, indices, repeat=True)
            if return_x0:
                x0_l = torch.cat(x0_l, dim=0)
                x0_l = self.order_all_frames(x0_l, indices, repeat=True)
                indices = list(range(self.total_frame_number))
                return latents, indices, controls, x0_l
            indices = list(range(self.total_frame_number))
    ...
    @torch.no_grad()
    def reverse_diffusion(self, latents=None, control_image=None, guidance_scale=7.5, indices=None, real_x0=None):
        self.scheduler.set_timesteps(self.num_inference_steps, device=self.device)
        with torch.autocast('cuda'):

            if hasattr(self, 'shuffling_type') and '-' in self.shuffling_type:
                no_prompt_latents = latents
            ...
            for i, t in tqdm(enumerate(self.scheduler.timesteps), desc='reverse_diffusion'):
                ...
                logging.info(f"Timestep {t}, indices are {indices}")
                logging.info(f"Latents begin with {latents[0][0][:2,:2]}")

                if self.is_shuffle:
                    if hasattr(self, 'shuffling_type') and '-' in self.shuffling_type:
                        all_models = self.shuffling_type.split('-')
                        logging.info(f"Shuffle models are {all_models}, latents are {latents.shape}")
                        latents_lst, indices_lst, control_lst = [], [], []
                        no_prompt_lst = []
                        for type in all_models:
                            latents_i, indices_i, control_image_i, no_prompt_latents_i = pf.shuffle_latents(latents, control_image, indices, self.total_frame_number, self.sample_size,
                                   self.grid_frame_number, self.grid, type, self.clusters, self.similarities, no_prompt_latents)
                            logging.info(f"Denoised latents are {latents_i.shape} with indices {indices_i}")
                            logging.info(f"Latents are like {latents_i[0][0][:2,:2]}")
                            latents_lst.append(latents_i)
                            indices_lst.append(indices_i)
                            control_lst.append(control_image_i)
                            no_prompt_lst.append(no_prompt_latents_i)
                    else:
                        shuffling_type = self.shuffling_type if hasattr(self, 'shuffling_type') else 'mallowsAdaptive'
                        logging.info(f"Shuffle type is {shuffling_type}, latents are {latents.shape}")
                        latents, indices, control_image = pf.shuffle_latents(latents, control_image, indices, self.total_frame_number, self.sample_size,
                                   self.grid_frame_number, self.grid, shuffling_type, self.clusters, self.similarities)
    ...
    def order_all_frames(self, current_video, current_indices, repeat=False):
        # find the current indices of the frames in order
        if repeat:
            # redundant frames (last frames of the first grid and first frames of the last grid) are ignored
            to_order_indices = []
            for ind in range(self.total_frame_number):
                if current_indices.count(ind) > 1:
                    to_order_indices.append(current_indices.index(ind,self.grid_frame_number))
                else:
                    to_order_indices.append(current_indices.index(ind))
        else:
            to_order_indices = [current_indices.index(ind) for ind in range(self.total_frame_number)]
        # reorder frames of the video
        out_video_lst = []
        for j in range(self.sample_size):
            grid_indices = to_order_indices[j * self.grid_frame_number:(j + 1) * self.grid_frame_number]
            out_grid, _ = fu.prepare_key_grid_latents(current_video, self.grid, self.grid, grid_indices)
            out_video_lst.append(out_grid)
        out_video = torch.cat(out_video_lst, dim=0)
        return out_video
    ...
    def ddim_inversion(self, latents, control_batch, indices):
        k = None
        els = os.listdir(self.inverse_path)[:self.total_frame_number]
    ...
    def process_image_batch(self, image_pil_list):
        ...
        if self.sample_size != -1:
            img_torch, control_torch = img_torch[:self.sample_size], control_torch[:self.sample_size]
    ...
    def activate_token_merging(self):
        tm.apply_patch(self.unet, self.local_merge_ratio, seed=self.seed, batch_size=2, target_stride=self.grid_size**2)

    def video_grids_to_frames_list(self, indices, init_latents_pre):
        latent_grid_size = [3, 3]
        # retrieve all encoded frames
        tmp = init_latents_pre
        img_h, img_w = tmp.size(-2) // latent_grid_size[0], tmp.size(-1) // latent_grid_size[1]
        list_of_flattens = [fu.flatten_grid(el.unsqueeze(0), latent_grid_size) for el in tmp]
        long_flatten = torch.cat(list_of_flattens, dim=-1)
        list_frames = [long_flatten[:, :, :, ind * (img_w):(ind + 1) * (img_w)].cpu().flatten().reshape(-1, 1) for ind in indices]
        return list_frames
    ...
    def __call__(self, input_dict):
        ...
        if 'shuffling_type' in input_dict.keys():
            self.shuffling_type = input_dict['shuffling_type']
        self.give_control_inversion = input_dict['give_control_inversion']
        
        self.guidance_scale = input_dict['guidance_scale']

        logging.info(f"\n Video editing on {input_dict['video_name']}, with prompt {self.positive_prompts} and shuffle type {self.shuffling_type}")
        ...
        list_frames = self.video_grids_to_frames_list(indices, init_latents_pre)

        # preprocess
        self.clusters = None
        self.similarities = None
        if self.shuffling_type.startswith("kmeans"):
            self.clusters = pf.compute_clusters(list_frames, self.shuffling_type, self.total_frame_number, self.grid_frame_number)
        elif self.shuffling_type == "mallowsAdaptive":
            self.similarities = pf.compute_similarities(list_frames)
        ...
        # for token merging
        if 'local_merge_ratio' in input_dict.keys():
            self.local_merge_ratio = input_dict['local_merge_ratio']
            if self.local_merge_ratio != 0:
                self.encoded_source_frames = init_latents_pre
                self.activate_token_merging()

        self.cond_embeddings, self.uncond_embeddings = self.get_text_embeds(self.positive_prompts, self.negative_prompts)
        latents_denoised, indices, controls = self.reverse_diffusion(latents_inverted, control_batch, self.guidance_scale, indices=indices, real_x0=init_latents_pre)

        logging.info(f"Denoised latents are {latents_denoised.shape}, with indices {indices}")
